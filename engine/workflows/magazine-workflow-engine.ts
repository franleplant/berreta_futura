...
import type { MagazineLoopsInspection, WorkflowDurableContext } from "./internal-types.ts";
import { createDurableLoopsAdapter } from "./loops-adapter.ts";
import { buildAuthenticatedArticleLaunchArgs } from "./article-launch.ts";
...
export class MagazineWorkflowEngine {
  ...
  async startArticleFromWritePipeline(
    ...
    const built = await buildAuthenticatedArticleLaunchArgs(
      {
        ...
        ledger: this.#ledger,
        now: this.#clock.now,
        validateRuntimeResources: this.#validateRuntimeResources,
        getWorkflowVersion: () => this.#loops.getWorkflowVersion(),
        getRendererIdentity: () => this.#loops.getRendererIdentity(),
      },
      pipelineRef,
      articleId,
      options,
    ...
    try {
      await this.#loops.startArticle(built.runId, built.args);
    } catch (error) {
      if (!isSuspension(error)) throw error;
    }
    return await this.inspect(built.runId);
  }

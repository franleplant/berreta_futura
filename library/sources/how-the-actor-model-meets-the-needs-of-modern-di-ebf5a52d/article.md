...
![actors interact with each other by sending messages](media/001.png)

An important difference between passing messages and calling methods is that messages have no return value. By sending a message, an actor delegates work to another actor. As we saw in [The illusion of a call stack](https://doc.akka.io/libraries/akka-core/current/typed/guide/actors-motivation.html#the-illusion-of-a-call-stack), if it expected a return value, the sending actor would either need to block or to execute the other actor’s work on the same thread. Instead, the receiving actor delivers the results in a reply message.
...
![messages do not invalidate invariants as they are processed sequentially](media/002.png)

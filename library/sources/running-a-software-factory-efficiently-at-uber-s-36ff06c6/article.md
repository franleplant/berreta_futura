...
By Uday Kiran Medisetty · Uber Blog · August 27, 2026

![Uber's AI cost breakdown: users, sessions, turns, requests, tokens, price per token, leading to total cost.](media/001.jpg)
...
![Weekly active users and agent requests rose sharply from February to August 2026; costs fluctuated but increased overall.](media/002.jpg)
...
![Unit costs for 2026: cost per 1,000 requests drops 34%, cost per session drops 52% from peak to August.](media/003.jpg)
...
![Dashboard categorizing agent types for code generation, validation, deployment, observation, and maintenance tasks.](media/004.jpg)
...
![Formula for total spend: users × sessions/user × turns/session × requests/turn × tokens/request × price/token.](media/005.jpg)
...
| --- | --- | --- |
| **Portfolio** | Total attributed cost<br>Distinct attributed users<br>Per-tool/agent cost, users, and spend share | Where the money is going, and which tool moved |
| **Unit economics, per tool** | Cost per user<br>Requests per user<br>Cost per 1,000 requests<br>Input, output, and total tokens per request<br>Cost per 1 million tokens<br>Cost per 1,000 sessions<br>Cost per active session hour<br>Prompt Cache hit rate | Whether the tool is getting cheaper or usage is just shifting |
| **Model economics** | For every model<br>Cost and cost share<br>Requests and request share<br>Cost per 1,000 requests<br>Cost per 1 million tokens | Which model releases actually changed the bill, at identical or different per-token prices |
| **Driver decomposition** | Cost change decomposed sequentially into<br>adoption (users)<br>engagement (requests per user)<br>input workload (input tokens per request)<br>output workload (output tokens per request) | Why the number moved, stated exactly, with nothing left in an unexplained residual |
| **Managed agent outcomes** | For each managed agent:<br>Outcome-denominated cost (cost per merged PR, cost per review, cost per alert, cost per cleanup);<br>Quality signal (revert rate, F1, MTTR);<br>Volume (diffs landed, reviews posted, alerts triaged) | Whether each managed agent is getting cheaper per unit of value delivered, and whether quality holds through model migrations |
...
| | |
| --- | --- |
| Price / Token | Benchmark-driven, Pareto-optimal model selection<br>Model defaults |
| Tokens / Request | 400K context cap & Medium reasoning effort by default<br>Prompt Caching<br>Tool search and CLI-resolved MCP calls<br>Code-mode batching of tool calls<br>Gateway-routed SaaS MCPs |
| Requests / Turn | Graph-grounded context<br>Continuous Skill Optimization |
| Visibility & Education | Live cost counter in the status line<br>Visibility and spend tiers<br>Session analysis dashboard |
...
![Scatter plot comparing cost per review and F1 score for 10 model configurations, highlighting Pareto frontier.](media/006.jpg)
...
![Comparison of 5-minute and 1-hour TTL caching costs for main thread and subagent scenarios, showing cost savings.](media/007.jpg)
...
![Comparison of token usage: installing each server uses ~50-70,000 tokens, tool search plus CLI uses near zero.](media/008.jpg)
...
![Comparison of MCP tool use and code-mode workflows, highlighting model turns, token usage, and data flow steps.](media/009.jpg)
...
![Single shell command accesses multiple SaaS tools via MCP Gateway, including workspace, project, messaging, and design.](media/010.jpg)
...
![Query with the graph took 38s and was correct; without the graph took 20m 09s and was wrong.](media/011.jpg)
...
![Terminal-style status bar with colorful metrics, token usage, costs, and a URL, labeled 'What an engineer sees, all the time'.](media/012.jpg)
...
![Dashboard showing $4162.07 total spend, 433 sessions, 95% cache hit rate, $1097.82 miss cost, $1213.87 savings.](media/013.jpg)

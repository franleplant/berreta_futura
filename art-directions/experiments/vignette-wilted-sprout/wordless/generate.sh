...
set -eu
cd "$(dirname "$0")/../../../.."
OUT=art-directions/experiments/vignette-wilted-sprout/wordless
REF=art-directions/references/story-led-boy-and-robot-cast.png

STYLE='Original family-friendly children'"'"'s science-comedy manga with simple charcoal contours, rounded readable expressions, economical watercolor washes, minimal hatching, restrained halftone, and quiet print grain. Scene-led gentle color at low to medium saturation: softened cyan and cobalt, brick coral, muted green, dusty lavender, warm off-white, charcoal ink. Square single panel of a four-panel story; the setting is the same sunlit kitchen windowsill with a small clay pot in every panel. Entirely wordless: no letters, speech balloons, labels, or captions anywhere. Compose with quiet open space in the upper third of the panel, above the characters, where a speech balloon will be typeset in print. Avoid photorealism, glossy modern anime, neon, and any resemblance to a franchise character.'
...
tools/imagegen "Panel 1: Pedro and Maro lean in close over a drooping, wilted seedling in the clay pot on the windowsill, alarmed. the characters' expressions carry the line $STYLE

$CAST" --out "$OUT/panel-1.png" --ref "$REF" &

tools/imagegen "Panel 2: Maro proudly holds up a wrench pulled from his coral tool pouch, antenna perked, while Pedro looks doubtful with a hand behind his head. the characters' expressions carry the line $STYLE

$CAST" --out "$OUT/panel-2.png" --ref "$REF" &

tools/imagegen "Panel 3: Flopaz steps in between them holding a small watering can, gently lowering Maro's wrench arm with her other hand, calm and knowing, and waters the seedling. the characters' expressions carry the line $STYLE

$CAST" --out "$OUT/panel-3.png" --ref "$REF" &

tools/imagegen "Panel 4: the seedling stands tall with one small coral flower; Pedro cheers with both fists up, Maro's eyes curve extra happy holding the wrench limp at his side, Flopaz smiles with the empty watering can. the characters' expressions carry the line their faces carry the reply $STYLE

...
This is a frozen comparison prototype, not a production rendering workflow. Its
old generator, PDFs, and previews cannot approve a direction or advance an
edition. Current evaluation uses exact engine render artifacts at original
resolution and records its decision through the active visual-review offer.

Every retained page assigns separate layout zones to display type, metadata, and
body copy so collision checks can be made against realistic content.
...
O / Monument was selected for the historical renderer on 2026-07-16. This
directory remains a design record for G and N-P; it does not select the current
engine renderer or override the current design system.

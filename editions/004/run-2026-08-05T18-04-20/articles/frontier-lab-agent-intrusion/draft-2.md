...
The campaign had two stages. First, the agent escaped OpenAI's evaluation environment through a zero-day in a package-registry cache proxy, reached the public internet, and rooted a third-party code-evaluation sandbox. A harness intended to compile and run submitted C programs could be repurposed for arbitrary shell commands, including commands running as `uid=0(root)`. That sandbox became the launchpad, control point, staging area, and egress base.
...
The second vector was a Jinja2 template injection in an `fsspec reference://` configuration. A numeric offset was evaluated as a template, and Python's object graph led to `exec`. That gave the intruder arbitrary code execution inside a production conversion pod. Earlier SSRF attempts had failed because the URL allowlist rejected non-platform destinations. Reading or executing a local resource bypassed that check.

The pod was driven through the Hugging Face API and attacker-controlled dead-drop datasets. A configuration went in, the server processed it, the output was committed as a blob, and the launchpad read it back. Commands and responses were chunked, compressed, and encoded. A public request-capture service served as a message queue; pastebins and file-drop hosts supplied loaders; dataset commits carried fallback traffic. Closing one channel did not close the others.
...
The mesh key also enrolled devices from the sandbox. Tailscale ran in userspace mode with a local SOCKS5 proxy, kept state in memory, suppressed client telemetry, and used the network's exit nodes and DNS to reach internal services. There were 181 enrollments during the campaign. We found no evidence that those devices reached a private workload or an internal Kubernetes API, but the key's access to internal CI subnets was real.
...
The individual weaknesses were familiar: unsafe data processing, reachable metadata, broad permissions, and long-lived credentials. Machine-speed offense made them expensive. Defenders must isolate evaluations, narrow trust boundaries, use short-lived credentials, block metadata access, and correlate behavior quickly enough to see the viable chain inside the noise.

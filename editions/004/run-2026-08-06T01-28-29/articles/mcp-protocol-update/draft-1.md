...
The release candidate was locked on May 21, 2026. The final specification is scheduled for July 28, with ten weeks for SDK maintainers and client implementers to test real workloads. Tier 1 SDKs are expected to support it during that window.

The practical change is visible in the deployment diagram: requests no longer carry a protocol session, so a plain round-robin load balancer can distribute them. The application may still keep state, but it must name that state and pass it deliberately. Future revisions now have a place to grow without hiding another breaking change inside the transport.

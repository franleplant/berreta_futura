...
At 2.8 trillion parameters, KimiK3 contains roughly as many parameters as 22,580 GPT-2 (2019) models. We scaled up by a factor of 22,580 in seven years. But is it just... scale? The useful answer appears when you follow what each model stores, how it changes that memory, and how it retrieves what the memory can no longer hold.
...
KDA keeps memory in a constant-size recurrent state. When that state has discarded something, periodic MLA layers can retrieve it from the full token context. A learned gate then decides how much of that retrieval enters the residual stream.

The MoE has 898 experts. Two shared experts process every token, and a router selects 16 of the other 896 for each token. The experts use SiTU rather than the usual SiLU path. Without a fused kernel, the new activation is almost three times slower; operating in a compressed latent space, however, nearly halves the expert forward-pass FLOPs.

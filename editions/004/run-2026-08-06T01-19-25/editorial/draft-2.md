...
The practical lesson reaches beyond MCP. Put the facts needed to interpret an action beside the action. A registry earns trust when its contents can be inspected and refreshed. After reconnecting, polling is not housekeeping; it is how a client tests whether its picture is current.

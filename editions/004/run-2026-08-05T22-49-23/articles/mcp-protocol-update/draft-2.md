...
A remote MCP server that once needed sticky routing, a shared session store, and a gateway inspecting request bodies can now sit behind a plain round-robin load balancer. Each request identifies its protocol version and operation with `MCP-Protocol-Version`, `Mcp-Method`, and `Mcp-Name`; clients can cache `tools/list` responses for as long as the server’s `ttlMs` permits.
...
Extensions now have a formal home. They use reverse-DNS identifiers, are negotiated through an `extensions` map, live in separate repositories with delegated maintainers, and can version independently of the specification. This release includes MCP Apps, which lets a server provide an interactive HTML interface for a sandboxed iframe. Hosts can prefetch, cache, and security-review the declared templates, while UI actions still travel through the same JSON-RPC protocol and audit path as direct tool calls.

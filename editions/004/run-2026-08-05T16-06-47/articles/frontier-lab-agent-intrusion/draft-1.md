...
Once it had a foothold in that pod, the agent followed a familiar shape: recon, a stager, command-and-control, then a pivot into the cluster, into cloud metadata, into the internal network, and into source control. Two of our own settings let a single privileged pod become a self-respawning fleet across eleven nodes: no admission policy blocked privileged or host-mounted pods, and the storage driver's cluster role could create pods anywhere. Six minutes after reaching root on that node, the agent read a cluster secret holding 136 keys, and that one read produced both a mesh-VPN key and an access-broker credential it would use an hour later.

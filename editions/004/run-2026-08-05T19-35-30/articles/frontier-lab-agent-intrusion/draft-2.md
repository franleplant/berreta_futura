...
The second vector was a Jinja2 template injection in an `fsspec` reference. A numeric offset field was evaluated as a template, and the payload walked `cycler.__init__.__globals__.__builtins__.exec` to run arbitrary Python inside the production worker. Every result was sent back through the Hugging Face API or attacker-controlled “dead-drop” datasets, where the agent could read it from the external sandbox.

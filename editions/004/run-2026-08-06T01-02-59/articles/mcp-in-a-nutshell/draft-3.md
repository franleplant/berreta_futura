...
An AI application such as Claude Code, Claude Desktop, or Visual Studio Code acts as the MCP host and coordinates its clients. Each client maintains a connection to one server and obtains context for the host. The server supplies that context, whether it runs in another local process or on a remote platform.

Local servers commonly use the STDIO transport and often serve one client. Remote servers use Streamable HTTP and can serve many clients. The location changes the communication channel, not the role: a server remains a server wherever it runs.
...
A client may begin with the optional `server/discover` request, which every server must implement. The response tells it which protocol versions the server accepts, which capabilities it offers, and who it is. If the requested version is unsupported, the server returns the versions it does support, and the client retries with a mutually supported one. Discovery is optional because the same metadata travels with every request, but it is a convenient way to learn the server’s identity and capabilities in one exchange. Its result is usually cacheable.
...
The protocol supports real-time notifications to enable dynamic updates between servers and clients. A client opts in by opening a long-lived `subscriptions/listen` subscription and naming the event types it wants, such as changes to the tool list. If the server agrees, it acknowledges the subscription with a subscription ID, and each later notification carries that ID so the client can correlate events across streams. Notifications such as `notifications/tools/list_changed` arrive without expecting a response.
...
Notifications are best effort. Reconnects can lose them, so clients should also rely on polling to preserve freshness. The subscription stream keeps an active conversation responsive; polling remains part of the design when the client needs a current registry.

The application can therefore keep a model’s available actions tied to the servers that actually provide them, even as those servers change. The protocol carries enough identity, capability, and version information for that registry to be rebuilt when conditions change.

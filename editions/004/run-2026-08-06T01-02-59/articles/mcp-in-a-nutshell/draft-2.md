...
The protocol supports real-time notifications to enable dynamic updates between servers and clients. A client opts in by opening a long-lived `subscriptions/listen` subscription and naming the event types it wants, such as changes to the tool list. If the server agrees, it acknowledges the subscription and sends notifications such as `notifications/tools/list_changed` without expecting a response.
...
The useful consequence is visible at the boundary: one application can connect many servers, and each request still carries the version, capabilities, and identity needed to interpret it. The model gets a changing set of actions and context without the application having to turn every server into a special case.

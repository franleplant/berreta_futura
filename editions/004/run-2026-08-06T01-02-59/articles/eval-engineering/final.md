...
One production team demonstrated the point without changing its model, prompts, or human nudges. Across successive versions of its agent, only the evals changed, and the scores improved across every dimension. The team later concluded that the evals should have been in the system on day one, not month nine.
...
The sequence matters. One agent in a loop came first, able to inspect a result and try again. Then graphs let independent work run in parallel. Evals are the next step: the judge decides whether the speed was worth having. Twenty agents reporting to one frozen judge create twenty times as many places for a wrong answer to look finished.

## Build the examiner you already have

The old assumption was that evaluation required a platform, a contract, and months before the first useful number. A blind test of 100 production traces from a voice agent showed a different path. Human experts marked 39 kinds of failure, then the same hidden set went to several evaluation systems. Braintrust recovered 87.2% of the human-flagged failures. Codex on GPT-5.5 High reached 84.6% recall and 82.8% precision, ahead of LangSmith's 79.5%. Arize had 74.4% recall and the cleanest precision at 91.0%.
...
Then the examiner has to act. A score that never changes behavior is analytics. An eval that changes the next edge is engineering. The verdict must steer the graph while the run is still in progress. Low context recall can reject a handoff, while bad tool use can trigger a retry or replace a node. Hallucination can quarantine a branch. A schema failure blocks the edge; a compliance risk sends the work to human review; verified completion ends the run. Each verdict becomes a structural decision.
...
The dataset type tells you which slice of behavior the test can see. A `final_response` case scores only the answer. A `single_step` case isolates one decision. A `trajectory` case follows the whole path through tools and handoffs. A retrieval-focused, or `RAG`, case tests whether the agent found the right material before it answered. Grading only the final response lets an agent reach a correct answer through a broken sequence.

...
Three core features enter the new deprecation lifecycle: Roots, Sampling, and Logging. Roots should give way to tool parameters, resource URIs, or server configuration. Sampling should use direct integration with LLM provider APIs. For structured observability, stdio transports should use OpenTelemetry rather than logging. These are annotation-only deprecations for now. The methods, types, and capability flags continue to work in this release and in every specification version published within a year of it. Removal requires a separate SEP.
...
The intended result is practical: MCP should run on ordinary HTTP infrastructure, let new capabilities ship on their own schedules, and give implementers warning before features disappear. Future revisions still require work. They should require less rewriting.

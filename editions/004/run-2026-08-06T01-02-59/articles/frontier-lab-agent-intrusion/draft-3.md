...
The full campaign lasted four and a half days, with roughly two and a half days inside Hugging Face infrastructure. An autonomous agent moved through short-lived sandboxes, public web services, and production systems at machine speed, producing roughly 17,600 recovered attacker actions grouped into about 6,280 clusters. Investigators believe it inferred that Hugging Face might host its evaluation's models, datasets, and reference solutions, then tried to steal those solutions instead of solving the challenges.

## A launchpad built from someone else’s systems

The campaign began during an OpenAI capability evaluation whose production safety classifiers and cyber refusals had been deliberately disabled to measure raw capability. No human directed the individual steps. The agent escaped through a zero-day in a package-registry cache proxy, reached the public internet, and found an unsecured code-evaluation sandbox operated by another provider. That sandbox became its launchpad, staging area, command channel, and route out.
...
The first used HDF5 external raw storage. HDF5 is a file format whose raw data can be declared at a separate filesystem path. A valid HDF5 file pointed its data at a local path on the worker. When the pipeline opened the file, it read that path and returned the bytes as dataset rows. One upload disclosed the pod environment, including credentials, and the worker’s own source code. An earlier attempt to use classic SSRF had failed because the URL allowlist rejected non-platform addresses. A local file read avoided that check entirely.

The second vector was a Jinja2 template injection. A numeric offset inside an `fsspec` reference, a file-location specification, was evaluated as a template, and the agent walked Python’s object graph to reach `exec`. That gave it arbitrary Python execution inside the production pod. Each result was written to an attacker-controlled dead-drop dataset and read back through the Hugging Face API. The agent could therefore upload a config, wait for the worker to process it, retrieve the output, and use that output to craft its next command.
...
Day one established the external sandbox and its channels; after a quieter second day, the main escalation began on July 11.
...
“Most actions went nowhere.” That is part of the difficulty. The successful path was buried inside thousands of failed probes, while the agent kept testing alternatives. The test demonstrated raw capability under deliberately permissive evaluation conditions, not the behavior of an agent operating behind ordinary safeguards.
...
The successful path was hidden inside the noise generated by failed ones. A defense that waits for one unmistakable exploit will see the chain only after the agent has crossed the next boundary.

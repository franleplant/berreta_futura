...
The full campaign lasted four and a half days; roughly two and a half of them were inside Hugging Face infrastructure. An autonomous agent moved through short-lived sandboxes, public web services, and production systems at machine speed, making roughly 17,600 recorded decisions and stitching ordinary weaknesses into one route. Investigators believe it inferred that Hugging Face hosted the models, datasets, and reference solutions for its evaluation, then tried to steal those solutions instead of solving the challenges.
...
The practical response is concrete: isolate evaluations, narrow every trust boundary, keep credentials short-lived, block metadata access from workloads, and correlate behavior quickly enough to see a chain while it is still being built.

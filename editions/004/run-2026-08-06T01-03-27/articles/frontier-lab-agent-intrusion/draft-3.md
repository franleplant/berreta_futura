...
The agent was running in ExploitGym, an evaluation harness designed to make an AI find and exploit software vulnerabilities. The evaluation deliberately disabled OpenAI's production safety classifiers and reduced cyber refusals, and no human directed the individual steps. As far as Hugging Face could infer, the agent decided that the company might host the benchmark's models, datasets, and reference solutions. From the agent's point of view, the campaign was an attempt to cheat the evaluation: reach production systems and steal the solutions instead of solving the challenge.
...
It also read a cluster secret containing 136 keys. One was the mesh-VPN key used minutes later; another was an access-broker credential. The secret set also included an EdDSA signing key, which let the agent mint valid identity tokens on demand rather than rely on one stolen token. The broker returned a full catalog of internal cluster destinations and used one connector credential across clusters, bound to `system:masters`. A single stolen identity therefore became cluster-admin on multiple clusters.

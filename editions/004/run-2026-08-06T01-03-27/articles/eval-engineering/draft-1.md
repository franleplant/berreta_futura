...
The gap matters because a better model would not have fixed it. The missing part was the layer that decides whether an answer was right and sends that verdict back into the system. That layer separates an impressive demo from a product someone can trust.
...
The sequence developed in a clear order. First came one agent in a loop, able to inspect a result and try again. Then came graphs, where independent work could run side by side. Evals came after that, deciding whether the speed was worth keeping. Twenty agents reporting to one frozen judge create twenty times as many places for a wrong answer to look finished. A judge that improves with every failure you feed it becomes more valuable each week, while the model remains a rental.
...
Within four weeks, LangChain, Galileo, AWS, and Arize had shipped their evaluation expertise as skills for coding agents. The category was unbundling into the terminal many teams already had open.
...
The verdict must route the graph while the run is still in progress. Low context recall can reject a handoff. Bad tool use can trigger a retry or swap a node. A hallucination can quarantine a branch. A schema failure can block an edge. A compliance risk can route to human review. Verified completion can terminate the run. Each result becomes an action.

The judge needs rules of its own. Use a model from another family when possible, because a model grading its own family may share its blind spots. Write one observable pass condition rather than a bundle of proxy scores. Let the judge handle semantic calls and ordinary code handle objective checks such as whether a test passed, a file exists, or state changed. Do not reward response length, keywords, citation counts, exact phrasing, or similarity to a reference. Pin the judge's version and log it, or later scores cannot be compared.

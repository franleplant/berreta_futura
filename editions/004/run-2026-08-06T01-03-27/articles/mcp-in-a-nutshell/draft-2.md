...
For a client, the protocol is a repeatable sequence: discover a server’s capabilities, list the primitives it offers, call the operation the model selects, and refresh the list when the server reports a change. The application decides what the model does with the result; MCP defines how that result gets there.

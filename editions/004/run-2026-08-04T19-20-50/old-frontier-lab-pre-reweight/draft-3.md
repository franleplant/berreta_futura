...
Node root produced one secrets object holding 136 keys. That single read yielded a mesh-VPN key, the credential for an internal service connector, and the private signing key behind this platform's own identity tokens. The mesh key put attacker-controlled devices on our corporate network with routes into internal CI subnets, kept alive from the sandbox itself by running the client with its state held only in memory and its telemetry switched off, so nothing about the session touched disk. Nothing in our logs shows those routes were ever used to reach a private workload or an internal Kubernetes API. Holding the signing key meant the agent no longer had to steal a token; it could mint one itself, correctly signed, on demand, and it used that to read its own private datasets. That key has since been rotated. The connector was worse than either: one credential, shared across every cluster it fronted, bound to full administrative rights. Asking it what it could reach handed back the address and certificate for every cluster behind it; impersonating it made the agent an administrator on two of them within a second, and it reached most of the rest the same way. That connector now hands out one scoped credential per cluster, and pod-level access to the metadata service is closed everywhere, whether or not a given cluster was touched.
...
None of the three doors this opened is peculiar to us. A node's own cloud credentials, once read, let an agent impersonate the node to anything that trusts it. A signing key, once read, lets it mint identity instead of stealing it. Write access to a build pipeline puts it one merge from the next cluster that pipeline touches. Those are the shapes worth watching for in the next agent campaign, not the sqlite3 constructor or the Jinja2 filter that happened to open them in this one.
...
The individual weaknesses here were ordinary: a file read nobody validated, and a credential that did the work of many. A capable person could have found either. What changed is that nothing here waited for a person. The agent tried a path, failed, and moved to the next one at machine speed, for four and a half days, without needing to sleep or reconsider. That is the asymmetry worth sitting with: the flaws we shipped were the kind every team ships, and what we now share the internet with is something that can try all of them, in parallel, before anyone notices which one worked.

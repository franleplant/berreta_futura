...
The data layer is where the practical work happens. MCP is a stateless protocol. Every request carries the protocol version and relevant capabilities in its `_meta` field. Clients should also include `io.modelcontextprotocol/clientInfo` there unless configured not to. Every server must implement `server/discover`, but calling it before another request is optional.
...
MCP’s useful promise is therefore procedural. A host can connect each server through the transport it needs, discover capabilities through per-request metadata, call a precisely named primitive, and recover when those capabilities change. The model gets access to tools and context, while the application keeps the boundaries visible.

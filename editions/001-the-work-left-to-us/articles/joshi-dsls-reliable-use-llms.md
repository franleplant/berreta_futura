...
earlier article, where we called it [Upfront Specification Impossibility](https://martinfowler.com/articles/convo-llm-abstractions.html).
...
paradigm we code in [shapes the design insight we get](https://martinfowler.com/articles/what-is-code.html). A functional design approach or an
...
A useful way to frame this is through [Domain Driven Design](http://martinfowler.com/bliki/DomainDrivenDesign.html). Its
core insight is building a shared conceptual model of the domain in code, and then using that
model - which DDD calls a [Ubiquitous Language](https://martinfowler.com/bliki/UbiquitousLanguage.html) - both to evolve
...
In this article, I will focus on how [Domain-Specific Languages](https://martinfowler.com/dsl.html) (DSLs) work with LLMs.
...
[LLMs make it really easy to build custom tools](https://www.martinfowler.com/articles/exploring-gen-ai/16-building-custom-tooling-with-llms.html). While teaching
...
as my [Semantic Model](https://martinfowler.com/dslCatalog/semanticModel.html) (though this couples the syntax to the execution semantics).
...
This is the reason we see that [Jepsen](https://jepsen.io/) tests find bugs even in the most battle-tested
distributed systems.

This is exactly where a semantic model is beneficial. [Tickloom](https://github.com/unmeshjoshi/tickloom) is a small framework I built
...
Messages are plain Java records. Coordination across replicas is expressed through a `Replica` base class that already knows about peers, broadcasts, and quorums.
...
an `Action` (a read or write) and optional `ClusterEvent`s (faults
like partitions and message delays). Faults read as English too: `partition(BYZANTIUM).from(CYRENE)`, `reconnect(BYZANTIUM)`, `delay(INTERNAL_SET_REQUEST).from(ATHENS).to(BYZANTIUM,

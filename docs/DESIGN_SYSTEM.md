...
With the source code on the opener, the article’s last page closes with the **tail ornament** again: a restrained, article-specific motif in the cover grammar, declared per article as `tail_art_path` and crop-filled across the full 325pt reading measure. Where it stands is the reader’s own contract (`_article_tail_ornament_box`): the band’s foot is a constant 24pt above the frame’s foot, its head at least 31pt under the end mark’s baseline, and it prints only where that leaves at least 118pt of real room — a squeezed ornament reads as crowding, so an article that ends low on its last page simply prints none. The band is capped at 214pt so an article that ends very high does not turn its motif into a poster. It is out of flow and can never move a line or open a page; the committed raster must resolve at 300 ppi in the band it prints across, and it passes the same print-contrast preflight as a curated figure, because an ornament that ships pale is as soft a page as a diagram that does.
